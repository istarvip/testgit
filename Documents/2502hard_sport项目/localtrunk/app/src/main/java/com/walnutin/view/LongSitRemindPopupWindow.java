...
//			, "155cm", "160cm", "165cm", "170cm", "175cm", "180cm", "185cm", "190cm", "195cm", "200cm", "205cm", "210cm"};
	...
	public LongSitRemindPopupWindow(Activity context, OnClickListener itemsOnClick) {
		super(context);
		...
		materialActivity = (WatchLinkActivity) context;

		final List<String> minitues =new ArrayList<>();
		for (int i =10;i<=240;i++){
			minitues.add(String.valueOf(i)+"min");
		}
		loopView_height.setListener(new OnItemSelectedListener() {
			@Override
			public void onItemSelected(int index) {
				mMinitues=minitues.get(index);
				setMinituesSetting(mMinitues);
			}
		});
		loopView_height.setItems(minitues);
		//设置初始位置
		...
		height_cancel.setOnClickListener(new OnClickListener() {
			@Override
	...
	WatchLinkActivity materialActivity = null;
	private void setMinituesSetting(String mMinitues) {
		materialActivity.setMinituesSetting(mMinitues);
