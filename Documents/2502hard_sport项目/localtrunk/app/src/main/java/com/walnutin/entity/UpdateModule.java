...
import com.walnutin.util.OtherUtil;
    ...
    public void checkNewVersion(Activity activity) {
        ...
        new AsyncTask<String, Integer, String>() {
            ...
            protected void onPostExecute(String result) {
                ...
                if (autoUpdate.checkUpdateInfo() && autoUpdate.mServerVersion != preferenceSettings.getLong("ignoreVersion", 0)) {
                    // 注册广播
                    mActivity.registerReceiver(downloadBroadcastReceiver,
                            new IntentFilter(
                    ...
                    if (preferenceSettings.getLong(downloadId, -1) == -1) {
                        ...
                        builder.setNegativeButton("忽略该版本",
                                new DialogInterface.OnClickListener() {
                                    @Override
                                    public void onClick(DialogInterface dialog,
                                                        int which) {
        ...
        if (cursor.moveToFirst()) {
            ...
            switch (status) {
                ...
                case DownloadManager.STATUS_SUCCESSFUL:
                    //	XLog.i(TAG, "下载完成");
                    if (downloadBroadcastReceiver != null) {
                        mActivity.unregisterReceiver(downloadBroadcastReceiver);
                    }
                    installApk();
                    break;
