...
public class DataProcessing implements IDataProcessing {
    ...
    public void processingData(byte[] value) {
        ...
        if (bufferData.startsWith("68") && data.endsWith("16")) {
            ...
            Log.d(TAG, "processingData: "+bufferData.toString());
            ...
            } else if (head.toUpperCase().equals("8E")) {  // 拍照 按钮
                mIDataCallback.onResult(null, true, GlobalValue.DISCOVERY_DEVICE_SHAKE);
            } else if (head.toUpperCase().equals("87")) {
                dealData_87(bufferData);
            } else if (head.toUpperCase().equals("AC")) {
                dealData_ac(bufferData);
            }
            bufferData = "";
        }
    ...
    private void dealData_ac(String bufferData) {

        if (bufferData.substring(4, 6).equals("05")) {
            if (bufferData.substring(8, 10).equals("02")) {   //APP 读取设备页面的配置
                mIDataCallback.onResult(bufferData, true, GlobalValue.READ_PAGE_CONFIG);
            } else if (bufferData.substring(8, 10).equals("03")) { //APP 读取设备支持的页面配置
                mIDataCallback.onResult(bufferData, true, GlobalValue.CHECK_SUPPORT_PAGE_SUCCESS);
            }
        }
    ...
    private void dealData_86(String bufferData) {
        ...
        Log.d(TAG, "dealData_86: "+ "进入到了处理86开头的逻辑");
        ...
        if (bufferData.length() < 20) {
            ...
            Log.d(TAG, "step: " + stepNum + " dis: " + distance + " cal: " + calNum);
    ...
    private void dealData_87(String bufferData) {
        mIDataCallback.onResult(bufferData, true, GlobalValue.Firmware_Version);
    }
    ...
    private void dealHeartData(String bufferData) {
        ...
        String serialString = bufferData.substring(16, 20); // 包号字符串 0803 08为总包数 03为第3个包
        HchDb.insertOrUpdateHeartRate(sf, serialString + data); // 添加原始心率数据以及包号到数据库

        if (true) {   // 暂时不执行 下面代码
            return;
        }
        int serial = Integer.parseInt(serialString.substring(2, 4)); // 包号
    ...
    private void dealSleep(String bufferData) {
        ...
        // data =     "FFEABDAFFFABCDFFFFFFFFFBFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFBAFFFFFFFFFFFFFFFF";
    ...
    private void parseSleep(String date, String beforeData, String data) {
        ...
        System.out.println("DataProcess: beforeData: " + beforeData);
        System.out.println("DataProcess: ToDayData: " + data);
        String filter;
        if (TextUtils.isEmpty(beforeData)) {
            filter = data;
        } else {
            filter = beforeData + data;
        }
        int filterFlag = 0;
        filter = DigitalTrans.hexStringToBinary(data);
        int filterlen = filter.length();
        for (int i = 0; i < filterlen / 2; i++) {
            String wake = filter.substring(i * 2, i * 2 + 2);
            if (!(wake.equals("11") || wake.equals("00"))) {
                filterFlag++;
            }
        }
        System.out.println("DataProcess: filterFlag: " + filterFlag);
        if (filterFlag < 6) {
            return;
        }
        ...
        if (TextUtils.isEmpty(beforeData)) {
            for (int i = 0; i < 27; i++) {
                int index = 54 - i * 2;
                String wake = data.substring(index, index + 2);
                if (!(wake.equals("FF") || wake.equals("00") || wake.equals("F0"))) {//|| !wake.equals("F0"
                    if (index > 2) {
                        String beforWake = data.substring(index - 2, index);
                        if ((beforWake.equals("FF") || beforWake.equals("00") || beforWake.equals("F0"))) {
                            continue;
                        }
                    }
                    wakeIndex = 27 - i;
            ...
            for (int i = 0; i < 27; i++) {
                ...
                if (!(wake.equals("FF") || wake.equals("00") || wake.equals("F0"))) {

                    String afterSleep = data.substring(index, index + 2);
                    if (afterSleep.equals("FF")) {
                        continue;
                    }
            ...
            for (int i = 27; i < 63; i++) { // 昨日18点 - 今天 18点 区间
                ...
                if (!(wake.equals("FF") || wake.equals("00") || wake.equals("F0"))) {
                    String afterSleep = data.substring(index, index + 2);
                    if (afterSleep.equals("FF") || afterSleep.equals("00") || afterSleep.equals("F0")) {
                        continue;
                    }
...
//            System.out.println("  timePointArray: " + newtimePointArray.get(i));
    ...
    private void dealOfflineStep(String bufferData) {
        dealStep(bufferData);
        mIDataCallback.onSynchronizingResult(bufferData, true, GlobalValue.OFFLINE_STEP_SYNCING);
    }
