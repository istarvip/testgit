...
import com.walnutin.hardsdkdemo.utils.DeviceHomeDataSp;
...
public class SleepModelImpl {
    ...
    public SleepModel loadTodaySleepModel() {
        ...
        if (today || laststampTime == 0) {
            sleepModel = (SleepModel) Conversion.stringToObject(mPedometerSettings.getString(HardSdk.getInstance().getAccount() + "_devSleep", null));
            if (sleepModel == null) {
                sleepModel = SqlHelper.instance().getOneDaySleepListTime(HardSdk.getInstance().getAccount(), TimeUtil.getCurrentDate());
            }
            if (sleepModel == null) {
    ...
    public void saveTodaySleepModel() {
        mPedometerSettings.setString(HardSdk.getInstance().getAccount() + "_devSleep", Conversion.objectToString(sleepModel));
    }
