...
import static com.walnutin.hardsdkdemo.utils.DigitalTrans.byteArrHexToString;
...
public class UUIDParser {
    ...
    public static String getServiceUUIDString(byte[] scanRecord) {
        ...
        while (index < scanRecord.length) {
            ...
            if (type == 3) {
                ...
                uuidString16 = parseUUID(type, data,0);
                Log.d("myresult", "getServiceUUIDString: type == 3 uuidString16:" + uuidString16);
            } else if (type == 6) {
                byte[] data = Arrays.copyOfRange(scanRecord, index + 1, index + length);
                uuidString16 = parseUUID(type, data,24);
                Log.d("myresult", "getServiceUUIDString: type == 6 uuidString16: " + uuidString16 + "  index: "+index+" length: "+length);
            } else if (type == 7) {
                byte[] data = Arrays.copyOfRange(scanRecord, index + 1, index + length);
                uuidString16 = byteArrHexToString(data);
                Log.d("myresult", "getServiceUUIDString: type == 7 uuidString16:" + uuidString16);
            }
            index += length;
        }
        return uuidString16;
    ...
    private static String parseUUID(int type, byte[] data,int startIndex) {
        if (data != null) {
        ...
        try {
            if (data != null) {
                ...
                String substring = uuidData.substring(startIndex, startIndex+4);
