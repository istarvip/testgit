...
//        mICommonImpl.startSyncBraceletData(isContainsSleepData);
    ...
    //有关数据查询的方法，请参考 manager包内的管理类和说明使用

    /**
     * 界面开发
     * 此方法只需要维亿魄手环调用
     * 维亿魄手环需要从手环内读取闹钟列表后，再进行设置
     * 此方法执行完，通过回调中GlobalValue.READ_ALARM_OK 确定读取完成，
     * 并读取回调第一个参数List<Clock> clockList 来获取读取出的闹钟列表。
     * 再进行设置
     */
    ...
    //以下包含is的功能需要在同步完手环后接收到同步完成命令后调用才有效。

    /**
     * 是否支持拍照
     * 包含is的功能需要在同步完手环后接收到同步完成命令后调用才有效。
     * @return
     */
     ...
     * // 是否支持 查找手机
     * 界面开发
     * 手环上点击查找手机图标， callback会收到GlobalValue.FIND_PHONE回调 ,
     * 界面在收到回调后响铃震动和弹窗提示，并确保软件在后台也能正确提示震动和响铃。
     *
     * @return
     */
